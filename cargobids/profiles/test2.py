...
pusher_client = pusher.Pusher(
    app_id = u"1053898",
    key = u"3e56e0d8aa7304c78a72",
    secret = u"08b56769fcd7758243ac",
    cluster = u"eu"
)
...
class UserViewSet(CommonViewset):
    ...
    def retrieve(self, request: Request, *args, **kwargs):
        if kwargs.get('pk') == 'me':
            ...
            response_data['notifications'] = NotificationSerializer(request.user.notifications, many=True).data
...
class QuoteViewSet(CommonViewset):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if isAirline(request.user):
            ...
            if scope is not None and scope == 'my':
                queryset = queryset.filter(bids__author=request.user).exclude(
                    bids__status='DELETED').distinct()
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        try:
            ...
            pusher_client.trigger(f'airlines', f'quote_created', {u'message': msg})
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            # push this new quote to all airlines
            msg = simplejson .dumps(response.data)
            pusher_client.trigger(f'airlines', f'quote_closed', {u'message': msg})
...
class BidViewSet(CommonViewset):
    queryset = Bid.objects.annotate(rank=(F('rate') + F('surcharges')) * F('cw_required')).order_by('rank', '-status')
    ...
    def perform_create(self, serializer):
        ...
        pusher_client.trigger(f'agent{quote.id}', f'bid{quote.id}', {u'message': u'bid has been created'})
...
def login(request):
    ...
    context = {'name': '{} {}'.format(user.firstname, user.lastname), 'content': 'Login Verifications Code:  ' + str(key)}
...
def registration_complete(request):
    ...
    context = {
        'content': 'New {} user has been registered on Cargobids with following information'.format(data['user_type']),
