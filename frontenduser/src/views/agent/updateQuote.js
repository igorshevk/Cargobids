...
class FormLayoutsUi extends Component {
  ...
  getQuote(quote_id) {
    api.getQuote(quote_id,(err, response)=>{
      ...
      let selected_area = {}, selected_origin = {}, selected_types = {};
      ...
      this.setState({dimensions:dimensions, quote:response, selected_origin:selected_origin, selected_area:selected_area, selected_types:selected_types});
...
//      chargeable = this.state.quote.kilos
  ...
  changeDimension = (e, index) => {
    ...
    //   this.checkPiecesValid(e, this.state.quote.pieces)
    ...
    if(this.state.dimensions.length) {
      try {
        if(parseInt(this.state.quote.total_pieces) !== parseInt(this.state.quote.pieces)) {
          errorNoti('Total number of pieces is not equal to N.Colli');
          return false;
        }
      } catch(e) {
        errorNoti('Total number of pieces is not equal to N.Colli');
  ...
  render() {
    ...
    return (
      <Fragment>
        <Row>
          <Colxx xxs="12">
            <h1><IntlMessages id="quote.update" /></h1>
              ...
              <CardBody>
                <Form className="quote-form">
                  ...
                  <Row>
                    ...
                    <Colxx sm={6} md={8}>
                        <button type="button" onClick = {() => this.addDimension()} className=" btn-shadow btn-multiple-state btn btn-success btn-lg">Add </button>&nbsp;
                        {dimensions.length ?<button type="button" onClick = {() => this.removeDimension()} className="btn-shadow btn-multiple-state btn btn-danger btn-lg">Remove</button>:''}
                    </Colxx>
                    {dimensions.length ?
                    <>
                    <Colxx sm={12}>
                    ...
                    <Colxx sm={12}>
                      ...
                      <Row>
                        <Colxx md={4} xs={12}>
                          <h6><strong>Total number of pieces : {quote.total_pieces} </strong></h6>
                        </Colxx>
                        <Colxx md={4} xs={12}>
                          <h6><strong>Total Volume : {quote.net_volume} </strong></h6>
                        </Colxx>
                        <Colxx md={4} xs={12}>
                          <h6><strong>Chargeable Weight : {quote.chargeable_weight} </strong></h6>
                        </Colxx>
                      </Row>
                    <br/> <br/>
                    </Colxx></> : ''}
                      ...
                      <FormGroup row>
                        <Label for="emailHorizontal" sm={4}>
                          <IntlMessages id="forms.deadline-date" />  &nbsp;
                           <span style={{padding: "1px" ,backgroundColor:'lightgrey' ,fontSize :'15px', color:"green",borderRadius:'30%',textAlign:'right',fontWeight: 'bold'}} href="#" id="Tooltipdeadline"> ? </span>
                            <Tooltip placement="right" isOpen={this.state.tooltipOpen} target="Tooltipdeadline" toggle={this.toggle}>
