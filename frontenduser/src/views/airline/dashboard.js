...
class DefaultDashboard extends Component {
  ...
  /*  renderCancelSubscription() {
    let canCancel = true,
      trial = false;
    let trial_end_date = this.addDaysToDate(
      this.state.users.subscription.current_period_start,
      this.state.users.subscription.trial_period - 1
    );

    if (
      this.state.users.subscription.canceled_by_user ||
    ...
    if (
      trial_end_date <= new Date() &&
      this.state.users.subscription.trial === 1
    )
      trial = true;

    this.setState({
      trial_end_date: getDateWithFormat(trial_end_date),
      canCancel: canCancel,
      trial: trial,
    });
  }*/
                  ...
                  <CardBody className="justify-content-between d-flex flex-row align-items-center">
                      ...
                      {canCancel || trial ? (
                        ...
                        <div>
                          ...
                          {this.state.users.days_left < 7 ? (
                            <a
                              onClick={this.toggle}
                              className="btn text-small btn-danger btn-sm text-white col-sm-12"
                            >
                    ...
                    <Modal isOpen={this.state.modal} toggle={this.toggle}>
                      ...
                      <ModalFooter>
                        <Button
                          color="info"
                          className="btn-shadow btn-multiple-state btn btn-info btn-lg"
