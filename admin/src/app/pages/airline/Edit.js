...
import {get, patch, AIRLINE_MODES} from '../../crud/api';
...
class AirlineEdit extends Component {
    constructor(props) {
        super(props);
        const {params: {airline_id}} = this.props.match;
            ...
            .then(
                (response) => {
                    ...
                    let companies = response.data.map((company) => {
                        ...
                        if(company.name.trim() == airline.airline_company_name.trim())
                            this.setState({selected_airline_company_name:company})
    ...
    getAirline = (airline_id) => {
        get('airlines/' + airline_id + '/')
            .then(
                (response) => {
                    AIRLINE_MODES.map(mode => {
                        if(mode.value == response.data.mode)
                            this.setState({selected_airline_mode:mode});
                    })
                    this.setState({airline:response.data})
                     this.getCompanies();
                });
    };
    ...
    handleSubmit = (e) => {
        ...
        patch('airlines/' + this.state.airline.id + '/', this.state.airline)
            .then(res => {
                if (res.data) {
        ...
        return (
            ...
            <div className="row">
                <div className="col-md-12">
                    <BoxContainerBasic
                        beforeCodeTitle={'Edit Airline ' +airline.airline_company_name}
